...
export ARCH=arm64
KERNEL_MAKE_ENV="LOCALVERSION=-BKLYN-Waipio-Gki-Kernel-"
...
variant(){
  findconfig=""
  findconfig=($(ls arch/arm64/configs/*_defconfig 2>/dev/null))
  declare -i i=1
  shift 2
  echo ""
  echo "${ON_BLUE}Variant Selection:${STD}"
  for e in "${findconfig[@]}"; do
    echo " $i. $(basename $e | cut -d'_' -f1)"
    i=i+1
  done
  local choice
  ...
  if [[ $i -gt 0 && $i -le ${#findconfig[@]} ]]; then
    ...
    export VARIANT=$(basename $v | cut -d'_' -f1)
...
anykernel3(){
  ...
  if [ -e $OUT_DIR/arch/arm64/boot/Image ]; then
    ...
    sed -i "s/ExampleKernel by osm0sis/${VARIANT} kernel by bklyn92/g" anykernel.sh
